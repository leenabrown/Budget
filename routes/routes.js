    ...
    if (req.session.menu === 'planned') {
      ...
      Budget.findOne({username: req.session.user, month: req.session.month, year: req.session.year}, function (err, result) {
        if (err) {
          ...
          result.insuranceBtn = '';
          result.debtBtn = '';
          result.gi = getIncome(result.giving);
          result.sa = getIncome(result.savings);
          result.ho = getIncome(result.housing);
          result.tr = getIncome(result.transportation);
          result.fo = getIncome(result.food);
          result.li = getIncome(result.lifestyle);
          result.ins = getIncome(result.insurance);
          result.de = getIncome(result.debt);
          var total = result.gi + result.sa + result.ho + result.tr + result.fo + result.li + result.ins + result.de;
          result.rem = Math.max(0, getIncome(result.income) - total);
        ...
        if (err) {
          ...
          Spent.findOne({username: req.session.user, month: req.session.month, year: req.session.year}, function (err, spent) {
            if (err) {
              ...
              var budgetSum = getIncome(budget.income); 
              var spentSum = getTotal(spent);
              var diff = parseFloat(budgetSum) - parseFloat(spentSum);
              result.inc = spentSum;
              result.rem = Math.max(0, diff);
...
var getTotal = function (spent) {
  var giving = spent.giving.slice();
  var savings = spent.savings.slice();
  var housing = spent.housing.slice();
  var trans = spent.transportation.slice();
  var food = spent.food.slice();
  var lifestyle = spent.lifestyle.slice();
  var insurance = spent.insurance.slice();
  var debt = spent.debt.slice();
  var total = 0;
  total = getIncome(giving) + getIncome(savings) + getIncome(housing) + getIncome(trans) + 
      getIncome(food) + getIncome(lifestyle) + getIncome(insurance) + getIncome(debt);
  return total;
}

var getIncome = function (income) {
  var total = parseFloat(0);
  for (var j = 0; j < income.length; j++) {
    total = parseFloat(total) + parseFloat(income[j].amount);
  }
  return total;
}
